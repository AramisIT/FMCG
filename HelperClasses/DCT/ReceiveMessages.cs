...
PIVOT (MAX(Count) for Type in([Acceptance],[Inventory],[Selection],[Movement])) as pivoted");
            ...
            if (result == null)
                ...
                {
                acceptanceDocCount = result[Processes.Acceptance.ToString()].ToString();
                inventoryDocCount = result[Processes.Inventory.ToString()].ToString();
                selectionDocCount = result[Processes.Selection.ToString()].ToString();
                movementDocCount = result[Processes.Movement.ToString()].ToString();
                }
            }
